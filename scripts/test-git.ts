...
let activeRepository:
  | ReturnType<typeof createStubRepository>
  | undefined = repo2;
...
(Module._load as unknown as (
  ...
  parent: Module | null,
  isMain: boolean,
) => unknown) = function mockedLoad(request, parent, isMain) {
  if (request === "vscode") {
    return {
      extensions: {
        getExtension: () => ({
...
try {
  ...
  const firstFile = (
    selectiveAdd?.files as Array<{ fsPath: string }>
  )[0];
  ...
  assert.equal(
    noActiveError,
    true,
    "Expected getRepo to fail when no active repository is selected",
  );
