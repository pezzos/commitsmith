...
cliMock.install((request) => {
  ...
  if (
    request.endsWith("pipeline") ||
    request.endsWith("pipeline.js")
  ) {
    return {
      runPipeline: async () => {
...
cliMock.queueError("Codex offline");

const repoDir = await mkdtemp(
  path.join(os.tmpdir(), "commit-smith-offline-"),
);
...
try {
  ...
  await execFileAsync(
    "git",
    ["config", "user.email", "offline@example.com"],
    { cwd: repoDir },
  );
  await execFileAsync(
    "git",
    ["config", "user.name", "Offline Tester"],
    { cwd: repoDir },
  );
  ...
  const messageLines = fallbackMessage
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  assert.match(
    subjectLine,
    /^chore\([^\)]+\): commit updated files \[offline mode\]$/,
  );

  const listedFiles = messageLines
    .slice(1)
  ...
  assert.deepStrictEqual(
    normalizeOrder(listedFiles),
    normalizeOrder(expectedFiles),
  );

  assert.ok(
    outputLogs.some((line) =>
      line.includes("[OFFLINE ⚠️] Codex unavailable"),
    ),
  );
  assert.ok(
    outputLogs.some((line) =>
  ...
  await execFileAsync(
    "git",
    ["add", "lib/service.ts", "docs/api.md"],
    { cwd: repoDir },
  );
  ...
  pipelineOutcomes.push({
    ...
    commitAnnotation:
      "[pipeline failed at tests: see OUTPUT > CommitSmith]",
  });
  ...
  assert.equal(
    pushAttempts,
    0,
    "Auto push should remain suppressed during commit-anyway fallback",
  );
