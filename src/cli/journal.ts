...
if (require.main === module) {
  runCli().catch((error) => {
    console.error(
      `CommitSmith CLI failed: ${(error as Error).message}`,
    );
    process.exitCode = 1;
  });
}
...
function parseMetaUpdates(args: string[]): Record<string, unknown> {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    metaEntries[key] = coerceMetaValue(raw);
  }
  return metaEntries;
}

function coerceMetaValue(raw: string): unknown {
  ...
  if (
    ...
    try {
      ...
      // Fall through to return the original string when JSON parsing fails.
    }
  }
