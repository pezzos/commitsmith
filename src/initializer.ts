...
export async function getInitializationStatus(
  ...
  const [journalReady, gitignoreReady, guidanceReady] =
    await Promise.all([
      isJournalReady(root),
...
async function isJournalReady(root: string): Promise<boolean> {
  ...
  try {
    ...
    throw new Error(
      `CommitSmith journal invalid: ${(error as Error).message}`,
    );
  }
}
...
async function ensureGitignoreRule(
  root: string,
): Promise<InitializationStepResult> {
  const gitignorePath = path.join(root, GITIGNORE_FILENAME);
  let content = "";
  try {
