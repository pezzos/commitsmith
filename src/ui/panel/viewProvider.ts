  ...
  implements vscode.WebviewViewProvider, vscode.Disposable
{
  ...
  private readonly themeTokensJson: string;

  constructor(
    private readonly context: vscode.ExtensionContext,
    private readonly deps: UiInfrastructureDeps,
  ) {
    this.themeTokensJson = this.loadThemeTokens();
  ...
  private async handleMessage(
    ...
    switch (message.type) {
      ...
      case "EXPORT_THEME": {
        await this.exportThemeVariables(message.payload?.variables);
        break;
      }
      default:
        break;
  ...
  private renderDocument(
    ...
    const tokensScript = `<script type="application/json" nonce="${nonce}" id="commit-smith-theme-tokens">${escapeForInlineScript(this.themeTokensJson)}</script>`;
    return {
      head: [
  ...
  private async exportThemeVariables(
    ...
    const rootLines = [
      "/* Auto-generated by CommitSmith. Do not edit manually. */",
      ":root {",
    ...
    const lines = [...rootLines, ...containerLines];
    ...
    try {
      ...
      output.appendLine(
        `CommitSmith: exported ${entries.length} theme variables to media/theme-mock.css`,
      );
      try {
        const stats = await fsp.stat(targetPath);
        output.appendLine(
          `CommitSmith: theme-mock.css now ${stats.size} bytes (${targetPath})`,
        );
        const tail = await fsp
          .readFile(targetPath, "utf8")
          .then((content) => content.split(/\n/).slice(-6).join("\n"));
        output.appendLine("CommitSmith: theme-mock.css tail:\n" + tail);
        const mirrored = await this.mirrorSnapshotToWorkspace(targetPath);
        if (mirrored.length > 0) {
          output.appendLine(
            "CommitSmith: mirrored theme snapshot to:\n" + mirrored.join("\n"),
          );
        } else {
          output.appendLine(
            "CommitSmith: no workspace copy of media/theme-mock.css was updated.",
          );
        }
      } catch (verifyError) {
        output.appendLine(
          `CommitSmith: verification of theme-mock.css failed – ${verifyError instanceof Error ? verifyError.message : String(verifyError)}`,
        );
      }
      void vscode.window.setStatusBarMessage(
        "CommitSmith theme snapshot exported.",
        2500,
      );
    } catch (error) {
      const message =
      ...
      const output = getOutputChannel();
      output.appendLine(
        `CommitSmith: failed to export theme snapshot – ${message}`,
      );
      console.error(
        "[CommitSmith] Failed to write theme snapshot:",
        message,
      );
      void vscode.window.showWarningMessage(
        "CommitSmith could not write media/theme-mock.css. See console for details.",
      );
    }
  }

  private loadThemeTokens(): string {
    const targetPath = path.join(
      this.context.extensionPath,
      "media",
      "theme-tokens.json",
    ...
    try {
      ...
      const output = getOutputChannel();
      output.appendLine(
        "CommitSmith: unable to load media/theme-tokens.json – falling back to empty catalog.",
      );
      console.warn(
        "[CommitSmith] Unable to load theme token catalog:",
        error,
      );
      return "[]";
    }
  ...
  private async mirrorSnapshotToWorkspace(
    sourcePath: string,
  ): Promise<string[]> {
    ...
    await Promise.all(
      vscode.workspace.workspaceFolders.map(async (folder) => {
        const candidate = path.join(folder.uri.fsPath, "media", "theme-mock.css");
        try {
          await fsp.mkdir(path.dirname(candidate), { recursive: true });
          await fsp.writeFile(candidate, buffer);
          updated.push(candidate);
        } catch (error) {
          const output = getOutputChannel();
          output.appendLine(
            `CommitSmith: failed to mirror theme snapshot to ${candidate} – ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }),
    );
